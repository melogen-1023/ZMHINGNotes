...
def task(file):
    ...
    with open('E:\\vacollections\\1.json', encoding='utf-8') as f:
    ...
    for data in datas:
        ...
        file += str(hour).zfill(2) + ':' + str(minute).zfill(2) + ':' + str(sec).zfill(2) + ',' + str(minisec).zfill(
            2)  # 将数字填充0并按照格式写入
        file += ' --> '
        ...
        file += '\n' + content + '\n\n'  # 加入字幕文字
        i += 1
    with open('E:\\vacollections\\1.srt', 'w', encoding='utf-8') as f:
